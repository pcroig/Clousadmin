...
- ❌ DO NOT apply temporary patches or superficial solutions
- 🔍 Investigate thoroughly before implementing

### 2. CLEAN CODE
- ✅ Clean, efficient, and organized code for **long-term** maintainability
...
### 4. ORGANIZATION
- ✅ Everything must be **ordered and well-structured**
- ✅ Reduce complexity through separation of concerns
- ✅ Follow consistent naming conventions
- ✅ Document important architectural decisions

### 5. CODE REUSE & SCALABILITY
- ✅ **Before duplicating code**, search if a similar solution already exists
- ✅ **Extract to reusable functions/components** when:
...
- ✅ **BEFORE making any change**, review **ALL relationships and dependencies**
- ✅ Identify **ALL places** that use the modified code/component/schema
- ✅ Verify that changes don't break existing functionality in other parts
...
- ✅ Use search tools (`Grep`, `Glob`, `Task` with Explore agent) to find ALL references
- ✅ Consider cascade effects: schema changes affect migrations, types, validations, queries
- ✅ **DO NOT assume** a change is isolated - always verify the entire system

**MANDATORY PROCESS when making changes:**
1. 🔍 **Search for all references** of the code to modify
2. 📋 **List all affected files** (direct and indirect)
3. ✅ **Verify compatibility** in each affected location
4. 🔧 **Update ALL necessary places** consistently
5. 🧪 **Consider edge cases** that might break

**When integrating external code or libraries:**
- ✅ **Verify tech stack compatibility** before integration (does it match Next.js 14, Prisma, TypeScript?)
- ✅ **Ask about project structure** if unclear (monorepo? single app? where is the code located?)
- ✅ **DO NOT assume** project structure - verify paths and conventions match
- ✅ **Check all dependencies** - ensure they're compatible with existing stack
- ✅ **Test integration** in isolation before applying system-wide

**ANTI-PATTERNS TO AVOID:**
- ❌ Quick patches without understanding the problem
- ❌ Duplicated or inconsistent code
- ❌ Superficial error handling (`try-catch` without a plan)
- ❌ Solutions that only work "for now"
- ❌ Lack of separation of concerns
- ❌ **Making changes without reviewing relationships and dependencies** ⚠️ **CRITICAL**
- ❌ Assuming a change is isolated without verifying impacts in other parts
- ❌ Modifying schemas/interfaces without updating all consumers
- ❌ Changing data formats without migrating existing code that uses them
- ❌ **Duplicating code** instead of extracting to reusable functions/components
- ❌ Creating specific components when they could be generic
- ❌ Ignoring optimization opportunities (unnecessary re-renders, inefficient queries)
- ❌ Hardcoding values that should be configurable or reusable
...
├── (dashboard)/      # Protected routes (empleado, hr)
├── api/              # API Routes (RESTful conventions)
└── layout.tsx

components/
├── ui/               # shadcn/ui components (auto-generated, don't edit manually)
├── empleado/         # Employee-facing components
├── hr/               # HR admin components
└── shared/           # Shared components (sidebar, notifications)
...
├── ia/               # AI processing logic (extract contracts, nominas, matching)
├── calculos/         # Business logic (ausencias, fichajes, balance-horas)
└── validaciones/     # Validation utilities (IBAN, NIF, festivos)
...
### Tool Usage
- **Use `Grep` tool** (not bash grep) for searching code references
- **Use `Glob` tool** for finding files by pattern
- **Use `Read` tool** for reading files before editing
- **Use `Edit` tool** for precise code modifications
- **Use `Task` tool with Explore agent** for codebase exploration and understanding structure
- **Use `TodoWrite` tool** for planning and tracking multi-step tasks

### Workflow
- **Always read files** before editing to understand context
- **Search for references** using Grep before making changes to understand impact
- **Use parallel tool calls** when operations are independent (multiple reads, searches)
- **Create todo lists** for complex multi-step tasks to track progress
- **Commit frequently** with descriptive messages following conventional commits

### Communication Style
- **Concise and direct** - avoid unnecessary verbosity
- **Spanish for user-facing text** - English for code and technical terms
- **No emojis** unless explicitly requested
- **Use code references** with file:line format (e.g., `lib/auth.ts:42`)
...
11. **⚠️ MANDATORY SYSTEM-WIDE REVIEW** - BEFORE any change:
    - Use Grep/Glob tools to search for ALL references to the modified code/schema/component
...
12. **🔄 CODE REUSE & SCALABILITY** - Always prioritize:
...
13. **📋 Use TodoWrite for planning** - Create task lists for complex features to ensure nothing is missed
14. **🔍 Explore before implementing** - Use Task tool with Explore agent to understand codebase structure before making changes
