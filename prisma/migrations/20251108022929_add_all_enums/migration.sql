/*
  ...
  - The `estado` column on the `ausencias` table would be dropped and recreated. This will lead to data loss if there is data in the column.
...
-- CreateEnum
CREATE TYPE "EstadoAusencia" AS ENUM ('pendiente_aprobacion', 'en_curso', 'completada', 'auto_aprobada', 'rechazada', 'cancelada');
...
DROP COLUMN "estado",
ADD COLUMN     "estado" "EstadoAusencia" NOT NULL DEFAULT 'pendiente_aprobacion';
